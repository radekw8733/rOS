use x86::bits64::paging::{PDFlags, PDPTFlags};

use crate::{subsystems::SUBSYSTEMS, terminal::Console, assembly_macros, paging::x86::{PDPTTable, PDTable}};
...
fn dump_page_tables() {
    ...
    let pml4 = SUBSYSTEMS.read().paging.as_ref().unwrap().pml4;
    let tables = SUBSYSTEMS.read().paging.as_ref().unwrap().tables;
    let mut level = 1;
    ...
    let pdpts = tables.entries;

    for page in pdpts.iter() {
        if page.is_present() {
